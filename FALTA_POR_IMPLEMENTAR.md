...
**Origen:** `Query/sjb/base_datos_mejorada.sql`  
...
| MovimientosCaja      | Sí (caja) | Sí     | Ingresos/egresos. **Error en SQL:** falta coma entre `idConcepto` y `concepto`; FK a `Concepto` (tabla en migración `create_catalogos_tablas.sql`). |
...
1. **MovimientosCaja (líneas 1087–1088)** — **CORREGIDO**  
   Se añadió la coma entre `idConcepto UNIQUEIDENTIFIER NULL` y `concepto VARCHAR(100)` en `Query/sjb/base_datos_mejorada.sql`. La FK a `Concepto(idConcepto)` exige que la tabla `Concepto` exista; se crea en `backAppC/migrations/create_catalogos_tablas.sql`. Ejecutar ese script antes del script principal o asegurar el orden de migraciones.
...
- **base_datos_mejorada.sql** asume o referencia tablas que se crean en:
  - **create_catalogos_tablas.sql**: Concepto, ClasificacionConcepto, CatalogoTipoMovimiento, MotivoTraslado, MotivoNotaCredito.
- Conviene documentar el orden de ejecución de migraciones (por ejemplo en `README` o en un único script “master” que ejecute los scripts en orden) para evitar errores de FK.

---

*Documento generado a partir de la revisión de `Query/sjb/base_datos_mejorada.sql` y del código en `backAppC` y `adminSPA`.*
