-- Reparación Fase 3: si la migración falló a medias, completar lo pendiente (idempotente).
-- Ejecutar solo si fase3_hotel_housekeeping_anticipos.sql falló o quedó a medias.

-- 1. HotelHousekeeping (crear si no existe)
...
IF NOT EXISTS (
    SELECT 1 FROM sys.indexes WHERE name = 'IX_HotelHousekeeping_EmpresaEstado' AND object_id = OBJECT_ID('HotelHousekeeping')
)
BEGIN
...
PRINT 'Reparación Fase 3 hotel completada.';
