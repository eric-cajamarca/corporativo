...
async function obtenerPorId(req, res) {
    ...
    try {
        ...
        console.error('motivoNotaCredito.obtenerPorId:', error);
...
async function codigosSunat(req, res) {
    ...
    try {
        const codigos = motivoNotaCreditoService.obtenerCodigosSunat();
        res.status(200).json({ data: codigos });
    } catch (error) {
        console.error('motivoNotaCredito.codigosSunat:', error);
        res.status(500).json({ message: error.message || 'Error', data: undefined });
    }
}
