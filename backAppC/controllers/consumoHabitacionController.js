...
async function agregar(req, res) {
    ...
    try {
        ...
        const creado = await withPool(async (pool) =>
            consumoHabitacionService.agregar(pool, req.user.empresa, body, idUsuario)
        );
...
async function actualizar(req, res) {
    ...
    try {
        ...
        await withPool(async (pool) =>
            consumoHabitacionService.actualizar(pool, req.params.id, req.user.empresa, body)
        );
...
async function eliminar(req, res) {
    ...
    try {
        await withPool(async (pool) =>
            consumoHabitacionService.eliminar(pool, req.params.id, req.user.empresa)
        );
        res.status(200).json({ data: { ok: true } });
    } catch (error) {
...
async function limpiarHabitacion(req, res) {
    ...
    try {
        ...
        await withPool(async (pool) =>
            consumoHabitacionService.limpiarPorHabitacion(pool, req.user.empresa, idProductoHabitacion)
        );
        res.status(200).json({ data: { ok: true } });
    } catch (error) {
