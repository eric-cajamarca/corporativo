...
const crear_producto_compuesto = async function (req, res) {
  try {
    ...
    const resultado = await withPool((pool) =>
      productoCompuestoService.crearProductoCompuesto(pool, datos, req.user)
    );
...
const obtener_componentes = async function (req, res) {
  try {
    ...
    const resultado = await withPool((pool) =>
      productoCompuestoService.obtenerComponentes(pool, idProductoPadre, req.user.empresa, req.user)
    );
...
const actualizar_componentes = async function (req, res) {
  try {
    ...
    const resultado = await withPool((pool) =>
      productoCompuestoService.actualizarComponentes(
        pool,
        {
          idProductoPadre,
          componentes,
...
const eliminar_producto_compuesto = async function (req, res) {
  try {
    ...
    const resultado = await withPool((pool) =>
      productoCompuestoService.eliminarProductoCompuesto(pool, idProductoPadre, req.user.empresa, req.user)
    );
...
const calcular_stock_compuesto = async function (req, res) {
  try {
    ...
    const resultado = await withPool((pool) =>
      productoCompuestoService.calcularStockCompuesto(
        pool,
        {
          idProductoPadre,
