...
async function listar(req, res) {
    ...
    try {
        ...
        const resultado = await catalogoFormaPagoService.listar(pool, req.query);
...
async function obtenerPorId(req, res) {
    ...
    try {
        const pool = await sql.connect(dbConfig);
        const item = await catalogoFormaPagoService.obtenerPorId(pool, req.params.id);
...
async function crear(req, res) {
    ...
    try {
        const pool = await sql.connect(dbConfig);
        const creado = await catalogoFormaPagoService.crear(pool, req.body);
...
async function actualizar(req, res) {
    ...
    try {
        const pool = await sql.connect(dbConfig);
        await catalogoFormaPagoService.actualizar(pool, req.params.id, req.body);
...
async function eliminar(req, res) {
    ...
    try {
        const pool = await sql.connect(dbConfig);
        await catalogoFormaPagoService.eliminar(pool, req.params.id);
        res.status(200).json({ data: { ok: true } });
    } catch (error) {
