...
async function listar(req, res) {
    ...
    try {
        ...
        const resultado = await motivoTrasladoService.listar(pool, req.user.empresa, req.query);
...
async function obtenerPorId(req, res) {
    ...
    try {
        const pool = await sql.connect(dbConfig);
        const item = await motivoTrasladoService.obtenerPorId(pool, req.params.id, req.user.empresa);
...
async function crear(req, res) {
    ...
    try {
        const pool = await sql.connect(dbConfig);
        const creado = await motivoTrasladoService.crear(pool, req.user.empresa, req.body);
...
async function actualizar(req, res) {
    ...
    try {
        const pool = await sql.connect(dbConfig);
        await motivoTrasladoService.actualizar(pool, req.params.id, req.user.empresa, req.body);
...
async function eliminar(req, res) {
    ...
    try {
        const pool = await sql.connect(dbConfig);
        await motivoTrasladoService.eliminar(pool, req.params.id, req.user.empresa);
        res.status(200).json({ data: { ok: true } });
    } catch (error) {
