...
async function listar(req, res) {
  ...
  try {
    const resultado = await withPool(async (pool) => {
      ...
      return conceptoService.listar(pool, idEmpresaListar, req.query);
    });
...
async function obtenerPorId(req, res) {
  ...
  try {
    const item = await withPool(async (pool) => {
      ...
      return conceptoService.obtenerPorId(pool, req.params.id, idEmpresa);
    });
...
async function crear(req, res) {
  ...
  try {
    const creado = await withPool((pool) => conceptoService.crear(pool, req.user.empresa, req.body));
...
async function actualizar(req, res) {
  ...
  try {
    await withPool((pool) => conceptoService.actualizar(pool, req.params.id, req.user.empresa, req.body));
...
async function eliminar(req, res) {
  ...
  try {
    await withPool((pool) => conceptoService.eliminar(pool, req.params.id, req.user.empresa));
    res.status(200).json({ data: { ok: true } });
  } catch (error) {
