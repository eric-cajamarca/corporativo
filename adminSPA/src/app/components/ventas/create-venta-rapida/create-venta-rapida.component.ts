...
export class CreateVentaRapidaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  /** Defaults y arranque de venta rápida. */
  private inicializarVentaRapida(): void {
    setTimeout(() => this.aplicarDefaultsVentaRapida(), 400);
  }

  private actualizarIndicadorVentasGuardadas(): void {
    this.hayVentasGuardadas = this.ventaProvisionalUi.tieneSesionesGuardadas();
  }

  abrirRecuperarVentasGuardadas(): void {
    if (!this.ventaProvisionalUi.tieneSesionesGuardadas()) {
      if (typeof iziToast !== 'undefined') {
        iziToast.info({ title: 'Sin ventas guardadas', message: 'No hay ventas en curso guardadas.', position: 'topRight' });
      }
      return;
    }
  ...
  aplicarDefaultsVentaRapida(): void {
    ...
    const medioContado = (this.mediosPago || []).find(
      (m: { descripcion?: string; codigo?: string }) => {
        ...
        return d.includes('CONTADO') || cod === '001' || cod === '1';
      }
    );
  ...
  /** Una línea de pago con la forma elegida y el total actual (venta rápida). */
  sincronizarPagoRapidoConTotal(): void {
    const idEstadoPago = Number(this.ventas.idEstadoPago) || 2;
    if (idEstadoPago === 1 && this.cabeceraEsCreditoFinanciacion()) {
      return;
    }
    ...
    const idForma = Number(this.formaPagoRapidaId);
    const forma = this.formasPago.find((f: FormaPago) => Number(f.idFormaPago) === idForma);
    if (!forma?.idFormaPago) {
  ...
  onFormaPagoRapidaChange(): void {
    this.sincronizarPagoRapidoConTotal();
  }

  onComprobanteRapidoChange(idComprobante: string | number): void {
    this.cargarDatosComprobantePorId(idComprobante);
  }
  ...
  cobrarVentaRapida(): void {
    ...
    if (!this.ventas.idComprobante) {
      iziToast.warning({ title: 'Advertencia', message: 'Seleccione tipo de comprobante.' });
      return;
    }
    if (!this.tieneClienteParaVenta()) {
  ...
  onCondicionPagoRapidaChange(): void {
    this.guardarEstadoProvisional();
  }

  onEstadoPagoRapidoChange(idEstado: string | number): void {
    this.ventas.idEstadoPago = idEstado;
  ...
  private limpiarQueryDuplicarDesdeVenta(): void {
    this.router.navigate(['/ventas/rapida'], { replaceUrl: true });
  }
  ...
  this._documentosService.getFormasPago().subscribe({
    next: (response) => {
      ...
      if (efectivo) {
        ...
        if (!this.formaPagoRapidaId) {
          this.formaPagoRapidaId = efectivo.idFormaPago;
        }
      }
      setTimeout(() => this.aplicarDefaultsVentaRapida(), 100);
    },
    error: (err) => {
      this.formasPago = [];
    }
