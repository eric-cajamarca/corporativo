...
export class CreateVentaRapidaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    this.buscadorModalEl = document.getElementById('vr-buscadorModal');
  ...
  enfocarInputBuscadorModalVentas(): void {
    const intentar = () => {
      const el = document.getElementById('vr-buscador-modal-search');
  ...
  /** Defaults y arranque de venta rápida (sin cotización, vale ni duplicar desde listado). */
  ...
  seleccionaProducto(prod: any): void {
    ...
    const buscador = bootstrap.Modal.getInstance(
      document.getElementById('vr-buscadorModal')!
    );
    buscador?.hide();
  }
...
abrirModalPrecios(item: any) {
  ...
  abrirBuscadorModal(): void {
    const el = document.getElementById('vr-buscadorModal');
    if (!el) return;
