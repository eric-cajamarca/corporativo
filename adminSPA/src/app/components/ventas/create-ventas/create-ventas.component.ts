...
export class CreateVentasComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  /**
   * Tras conocer si la sesión es gestora: hotel solo aplica a comercio/rubro hotel, no a gestora.
   */
  private inicializarCarritoSegunContexto(): void {
    ...
    } else if (!this.esGestora && this.hotelPreloadVentaService.hasPreload()) {
  ...
  this._documentosService.getFormasPago().subscribe({
    next: (response) => {
      // Saldo a favor no va en el select: solo el botón «Usar saldo a favor».
    ...
    this._tablasSunatService.obtener_medios_pago().subscribe(
      (response) => {
        ...
        // Ocultar SAF del select de medios (cabecera / combos de venta).
  ...
  private validarAntesDeCobrar(validarCarrito = true): boolean {
    ...
    if (this.esGestora) {
      const lineaSinStock =
        validarCarrito && !this.permitirVentasNegativas
      ...
      if (!gestora.valido) {
        iziToast.warning({
          title: gestora.titulo || 'No se puede registrar',
