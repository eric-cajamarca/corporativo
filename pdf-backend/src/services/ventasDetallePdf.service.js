  ...
  return `
    <div class="bloque-comprobante-compra">
      <table class="tabla-cabecera-compra">
        <tr><td colspan="4"><strong>CLIENTE:</strong> ${escapeHtml(comp.cliente)}</td><td class="text-end"><strong>${escapeHtml(comp.estado)}</strong></td></tr>
        <tr><td colspan="2"><strong>R.U.C./DNI:</strong> ${escapeHtml(comp.ruc)}</td><td colspan="3" class="text-end"><strong>TOTAL DOCUMENTO S/.:</strong> ${fmtNum(comp.total)}</td></tr>
...
function construirHtmlVentasDetallado(datos) {
  ...
  const sinDatos =
    comprobantes.length === 0
      ? '<p class="sin-datos">No hay ventas registradas en el periodo seleccionado.</p>'
      : '';
...
module.exports = { construirHtmlVentasDetallado };
